...
(function($) {
    ...
    var methods = {
        ...
        init: function(opts) {
            ...
            $el = $(this);
            if($el.find('tx-editor-wrapper').length > 0)
            	return;
            var content =
            ...
            $icon.on('click.txeditor.icon', function(e) {
                e.preventDefault();
                methods[isFullscreen ? "minimize" : "expand"]($(e.target).closest(".tx-editor-wrapper"));
            });
            return this;
        },
        ...
        expand: function($node) {
            ...
            var $editor = $node.find(".tx-editor"),
            	$el = $node.find("input") || $node.find("textarea");
            ...
            transitions($editor, $el);
            return this;
        },

        minimize: function($node) {
        	var $editor = $node.find(".tx-editor"),
            	$el = $node.find("input") || $node.find("textarea");
    ...
    var transitions = function ($editor, $el) {
			relocate($editor);
            if (isFullscreen) {
				$el.focus();
				isFullscreen = false;
